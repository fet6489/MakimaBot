...
export function before(m, { conn, command }) {
    ...
    // Solo si el mensaje es uno de los comandos permitidos
    if (!comandosPermitidos.includes(m.text?.toLowerCase()?.trim())) return !0
    ...
    // Cada 5 veces que use un comando permitido, se activa
    ...
    // (Opcional) Si sube de nivel, pero no es el trigger de las 5 veces, sigue mostrando el mensaje clásico de levelup
